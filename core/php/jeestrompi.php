...
//}	
	...
	if (isset($result['StromPi-DateTimeOutput'])) {
	  ...
	  list($StromPiDateTimeOutput, $StromPiModecfg, $StromPiModeState, $StromPiOutputVoltage, $StromPiWide, $StromPiUSB, $StromPiLifePo4V, $StromPiLifePo4Charge,$strompistatus) = explode("|", $result['StromPi-StrompiStatusOutput']);
	  ...
	  //$eq->save(true);strompistatus
		  ...
		  if (sizeof($cmds) > 0) {
			foreach($cmds as $cmd) {
                ...
                } elseif  ($cmd->getLogicalId() == 'strompistatus') {
					cmd::byId($cmd->getId())->event($strompistatus);
