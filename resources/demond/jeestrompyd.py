	...
	try:
		...
		if y != "":
			...
			if y.find('ShutdownRaspberryPi') > 0:
				...
				time.sleep(1)
				read_strompi_status()
			if y.find('StromPiPowerfail') > 0:
				logging.debug('receive message StromPiPowerfail')
				time.sleep(1)
				StromPiPowerfail = 0
				read_strompi_status()
			if y.find('StromPiPowerBack') > 0:
				logging.debug('receive message StromPiPowerBack')
				time.sleep(1)
				StromPiPowerfail = 1
				read_strompi_status()
...
def read_socket():
	...
	while not JEEDOM_SOCKET_MESSAGE.empty():
		...
		try:
			...
			elif message['action'] == "changemode":
				logging.debug('demande de changemenet mode avec la nouvelle valeur : %s',message['mode'])
				changeModeStrompi(message['eqlogic'],message['mode'])
			else:
...
def changeModeStrompi(eqlogic=0, strompiMode=1):
	logging.debug('execution de la commande changemode')
	ser.write(str.encode('\x0D'))
	time.sleep(0.3)
	ser.write(str.encode('strompi-mode ' + strompiMode))
	time.sleep(0.3)
	ser.write(str.encode('\x0D'))
	logging.debug('Mise a jour du mode effectué: %s', strompiMode)
	jeedom_com.send_change_immediate({'StromPi-changemode' : 'true', 'eqlogic' : eqlogic})
...
def read_strompi_status(eqlogic=0):
	...
	StrompiStatusOutput = ' ' + str(strompi_mode_converter((int(sp3_modus)))) + '|' + str(output_status_converter((int(sp3_output_status)))) + '|' + str(sp3_ADC_OUTPUT) + '|' + str(sp3_ADC_Wide) + '|' + str(sp3_ADC_USB) + '|' + str(sp3_ADC_BAT) + '|' + str(batterylevel_converter(int(sp3_batLevel),int(sp3_charging)))+ '|' + str(StromPiPowerfail)
